...
template<typename DType>
inline void CLDLGrad(Tensor<cpu, 2, DType> dst,
                      const Tensor<cpu, 2, DType> &src_1,
                      const Tensor<cpu, 2, DType> &src_2,
                      const Tensor<cpu, 2, DType> &src_3,
                      const Tensor<cpu, 1, DType> &label) {
#pragma omp parallel for
  for (openmp_index_t y = 0; y < dst.size(0); ++y) {
    const index_t k = static_cast<int>(label[y]);
    for (index_t x = 0; x < dst.size(1); ++x) {
      if (x == k) {
        DType prod = (1-src_2[y][x])*(1-src_3[y][x]);
        dst[y][k] = -1 / std::max(src_1[y][x], (DType)FLT_MIN) * std::pow(prod, (DType)0.5);
      } else {
        dst[y][x] = 0;
      }
    }
...
template<typename DType>
inline void CLDL(Tensor<cpu, 1, DType> dst,
                 const Tensor<cpu, 1, DType> &src_1,
                 const Tensor<cpu, 1, DType> &src_2,
                 const Tensor<cpu, 1, DType> &src_3) {
  for (index_t x = 0; x < dst.size(0); ++x) {
    DType prod = (1-src_2[x])*(1-src_3[x]);
    DType p = std::pow(prod, (DType)0.5);
    dst[x] = p * std::log(std::max(src_1[x], (DType)(FLT_MIN)));
  }
}
template<typename DType>
inline void CLDL(Tensor<cpu, 2, DType> dst,
                 const Tensor<cpu, 2, DType> &src_1,
                 const Tensor<cpu, 2, DType> &src_2,
                 const Tensor<cpu, 2, DType> &src_3) {
...
#pragma omp parallel for
  for (openmp_index_t y = 0; y < dst.size(0); ++y) {
    CLDL(dst[y], src_1[y], src_2[y], src_3[y]);
  }
}
