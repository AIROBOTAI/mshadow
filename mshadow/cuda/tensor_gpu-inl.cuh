...
template<int x_bits, typename DType,  typename DstPlan, typename SrcPlan>
__global__ void CLDLKernel(DstPlan dst,
                          SrcPlan src_1, SrcPlan src_2, SrcPlan src_3,
                          index_t xmax) {
  const unsigned x_size = 1 << x_bits;
  const int y = blockIdx.x;
  // calculate normalizer, with writeback
  //XJ: note the usage of x_size and xmax
  for (unsigned x = 0; x < xmax; x += x_size) {
    const unsigned xindex = x + threadIdx.x;
    if (xindex < xmax) {
      DType p1 = powf((1 - src_2.Eval(y, xindex)) * (1 - src_3.Eval(y, xindex)), 0.5);
      DType p2 = powf((1 - src_1.Eval(y, xindex)) * (1 - src_3.Eval(y, xindex)), 0.5);
      DType p3 = powf((1 - src_1.Eval(y, xindex)) * (1 - src_2.Eval(y, xindex)), 0.5);
//      DType fa = 3 / (p1 + p2 + p3);
      dst.REval(y, xindex) = log(max(src_1.Eval(y, xindex), (DType)FLT_MIN)) * p1;
//      dst.REval(y, xindex) = log(max(src_1.Eval(y, xindex), (DType)FLT_MIN)) * p1 * fa;
      // if (p >= 0.7)
      //   dst.REval(y, x + threadIdx.x) = log(max(src_1.Eval(y, x + threadIdx.x), (DType)FLT_MIN));
      // else
      //   dst.REval(y, x + threadIdx.x) = log(max(src_1.Eval(y, x + threadIdx.x), (DType)FLT_MIN)) * p;
    }
  }
}

template<typename DType>
inline void CLDL(Tensor<gpu, 2, DType> &dst,
                 const Tensor<gpu, 2, DType> &src_1,
                 const Tensor<gpu, 2, DType> &src_2,
                 const Tensor<gpu, 2, DType> &src_3) {
  dim3 dimBlock(kBaseThreadNum);
  dim3 dimGrid(dst.size(0));
  CHECK_EQ(dst.shape_, src_1.shape_) << "CLDL shape mismatch";
  CheckLaunchParam(dimGrid, dimBlock, "CLDL");
  cudaStream_t stream = Stream<gpu>::GetStream(dst.stream_);
  CLDLKernel<kBaseThreadBits, DType>
      <<<dimGrid, dimBlock, 0, stream>>> (
      expr::MakePlan(dst),
      expr::MakePlan(src_1),
      expr::MakePlan(src_2),
      expr::MakePlan(src_3),
      dst.size(1));
   MSHADOW_CUDA_POST_KERNEL_CHECK(CLDLKernel);
}

template<int x_bits, typename DType, typename DstPlan, typename SrcPlan1, typename SrcPlan2>
__global__ void CLDLGradKernel(DstPlan dst,
                              SrcPlan1 src_1, SrcPlan1 src_2, SrcPlan1 src_3,
                              SrcPlan2 label, index_t xmax) {
  const unsigned x_size = 1 << x_bits;
  const int y = blockIdx.x;
  const int k = static_cast<int>(label.Eval(0, y));
  // calculate normalizer, with writeback
  for (unsigned x = 0; x < xmax; x += x_size) {
    ...
    if (xindex < xmax) {
      if (xindex == k) {
        ...
        DType p3 = powf((1 - src_1.Eval(y, xindex)) * (1 - src_2.Eval(y, xindex)), 0.5);
//        DType fa = 3 / (p1 + p2 + p3);
//        dst.REval(y, xindex) = -p1 * fa / max(src_1.Eval(y, xindex), FLT_MIN);
        dst.REval(y, xindex) = -p1 / max(src_1.Eval(y, xindex), FLT_MIN);
        // if (p >= 0.7)
        //       dst.REval(y, xindex) = -1 / max(src_1.Eval(y, xindex), FLT_MIN);
        // else
        //       dst.REval(y, xindex) = -p / max(src_1.Eval(y, xindex), FLT_MIN);
      } else {
        dst.REval(y, xindex) = 0;
      }
    }
  }
}
...
template<typename DType>
inline void CLDLGrad(Tensor<gpu, 2, DType> &dst,
                     const Tensor<gpu, 2, DType> &src_1,
                     const Tensor<gpu, 2, DType> &src_2,
                     const Tensor<gpu, 2, DType> &src_3,
                     const Tensor<gpu, 1, DType> &label) {
  dim3 dimBlock(kBaseThreadNum);
  dim3 dimGrid(dst.size(0));
  CHECK_EQ(dst.shape_, src_1.shape_) << "CLDLGrad: shape mismatch";
  CHECK_EQ(dst.size(0), label.size(0)) << "CLDLGrad: label shape mismatch";
  CheckLaunchParam(dimGrid, dimBlock, "CLDLGrad");
  cudaStream_t stream = Stream<gpu>::GetStream(dst.stream_);
  CLDLGradKernel<kBaseThreadBits, DType>
      <<<dimGrid, dimBlock, 0, stream>>> (
       expr::MakePlan(dst),
       expr::MakePlan(src_1),
       expr::MakePlan(src_2),
       ...
       expr::MakePlan(label),
       dst.size(1));
  MSHADOW_CUDA_POST_KERNEL_CHECK(CLDLGradKernel);
  cudaStreamSynchronize(Stream<gpu>::GetStream(dst.stream_));
}
